...
/// <summary>
/// Retrieved from a post at https://forum.unity.com/threads/automatic-creation-of-materials.542169/ on Feb 8, 2024
/// Original script by Hyperific, posted Oct 22, 2021
/// Adapted to function with the In-House MRO Shader workflow by default, but this can be reconfigured in the future
/// 
/// Updated on 2024-02-16:
/// Previously, Unity would become locked out if the destination folder did not exist, or if the expected images were not selected
/// 
/// Added a check to make sure that the destination folder exists.
/// Added a check to ensure the ser had selected at least one Color Map.
/// </summary>
...
public static class CreateComplexMaterialFromTexture
{
    ...
    static void CreateComplexMatFromTx(List<Texture2D> tx2D, Shader shader)
    {
        ...
        string materialSavingFilePath = "Assets/Materials/";
        ...
        */
        ...
        //via the inspector after generation, but 0.3 should get us a reasonable baseline for most instanes. 
        mat.SetFloat("_Roughness_Intensity", 0.3f);
        mat.SetFloat("_AO_Intensity", 1.0f);
        //Set the specular preservation to ON. This means that anything that has an alpha of 0 will still have data repesenting its reflected light
    ...
    //tokenizes a file name and drops the last token, which we expect to be an identier for the type of file/texture
